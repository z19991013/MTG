...
class GAT(torch.nn.Module):
    ...
    def __init__(self, dropout_rate, dropedge_rate, Argument):
        ...
        #self.transformer=MyViT((1,1,self.postprocess.postlayernum[-1]),n_patches=1,hidden_d=20,n_heads=2,out_d=1)
        self.lstm1=nn.LSTM(self.postprocess.postlayernum[-1],hidden_size=100,num_layers=1)
        self.lin1=nn.Linear(100,50)
        self.lstm2=nn.LSTM(50,20)
        self.risk_prediction_layer = nn.Sequential(
            nn.Linear(self.postprocess.postlayernum[-1], 1),
            #nn.LSTM(self.postprocess.postlayernum[-1], hidden_size=1, num_layers=1)
        #    #nn.Linear(50,1),
        )
    ...
    def forward(self, data, edge_mask=None, Interpretation_mode=False):
        ...
        #postprocessed_output = self.lstm1(postprocessed_output)[0]
        #postprocessed_output=self.lin1(postprocessed_output)
        #postprocessed_output=self.lstm2(postprocessed_output)[0]
        risk = self.risk_prediction_layer(postprocessed_output)
        #print(postprocessed_output.shape)
        #postprocessed_output = postprocessed_output.resize(6, 1, 1, 200)
        #risk=self.transformer(postprocessed_output)

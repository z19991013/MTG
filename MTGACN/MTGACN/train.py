...
from utils1 import train_test_split
from utils1 import makecheckpoint_dir_graph as mcd
from utils1 import TrainValid_path
from utils1 import non_decay_filter
from utils1 import coxph_loss
from utils1 import cox_sort
from utils1 import accuracytest
...
class CoxGraphDataset(Dataset):
    ...
    def get(self, idx):
        ...
        data_re = Data(x=data_origin.x[:,:2048], edge_index=data_origin.edge_index)
        #print('data_re',data_re.size)
        mock_data = Data(x=data_origin.x[:,:2048], edge_index=data_origin.edge_index, pos=data_origin.pos)
...
def Train(Argument):
    ...
    #Metadata = pd.read_csv('./Sample_data_for_demo/Metadata/clinical.tsv', sep='\t')
    Metadata = pd.read_csv('./Sample_data_for_demo/Metadata/yuhou.csv')
    ...
    test_loader = DataListLoader(TestDataset, batch_size=batch_num, shuffle=False, num_workers=1, pin_memory=True,
                             drop_last=False)
    train_loader = DataListLoader(TrainDataset, batch_sampler=train_batch_sampler, num_workers=1, pin_memory=True)
    #train_loader = DataListLoader(TrainDataset, batch_size=batch_num, shuffle=False, num_workers=8, pin_memory=True,drop_last=False)
    val_loader = DataListLoader(ValidDataset, batch_size=batch_num, shuffle=False, num_workers=1, pin_memory=True,
                            drop_last=False)
    ...
    #, device_ids=[0, 1], output_device=0)
    ...
    scheduler = StepLR(optimizer_ft,step_size=len(train_loader))#,max_lr=Argument.learning_rate,steps_per_epoch=len(train_loader),epochs=Argument.num_epochs)#max_lr=Argument.learning_rate,

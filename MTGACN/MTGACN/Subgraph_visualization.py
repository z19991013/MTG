...
def Whole_supernode_vis(WSI, Position_pd, Location_pd, Select_ID, core_dir, edge_mask_norm, row, col):
    ...
    WSI_node_idx = Position_pd['Unnamed: 0'].tolist()
    #WSI_node_idx = Location_pd['id'].tolist()
    ...
    pos_dict = zip(Position_pd['Unnamed: 0.1'].tolist(), X_Y_pos)
    #pos_dict = zip(WSI_node_idx, X_Y_pos)
...
def IG_TME_vis(IG_nodes, row, col, IG_edge_index, Position_pd, Location_pd, core_dir, Select_ID, edge_mask_norm, edge_mask):
    ...
    if len(Mid_IG_subgraph) > 0:
        ...
        if len(Mid_IG_subgraph_nodes) != 0:
            ...
            Mid_IG_subgraph_nodes.to_csv(os.path.join('/'.join(core_dir.split('/')[:-1]),core_dir.split('/')[-1],Select_ID+'_TME_subgraph_small_cap_subgraph_exist_again.csv'))
        else:
            print(Select_ID)
...
def Calculate_IG(d, d_count, row, col, edge_mask, edge_mask_norm, patient_target_dir,
    ...
    #print('pid',p_id)
    #WSI_dir = os.path.join('./Sample_data_for_demo/Raw_WSI/', '-'.join(p_id.split('-')[0:3]),'.svs')
    WSI_dir = os.path.join(r'G:/TEA-graph-master/TEA-graph-master/Sample_data_for_demo/ok/')
    #r''+p_id+'.svs')
    WSI_dir_list = os.listdir(WSI_dir)
    #print('lenwsi',len(WSI_dir_list))
    WSI_dir_list = [item for item in WSI_dir_list if p_id in item]
    #print('111',len(WSI_dir_list))
    print(p_id)
    ...
    Location_pd = pd.read_csv('./Sample_data_for_demo/Graph_test/TCGA/KIRC/original3/' + '_'.join(id_path.split('_')[0:1]) + '_node_location_list.csv')

    Mask_fig_IG = Whole_supernode_vis(WSI, Position_pd, Location_pd, id_path, patient_target_dir, edge_mask_norm, row, col)
...
def Subgraph_analysis_vis(Argument):
    ...
    calculate_loader = DataLoader(TestDataset, batch_size=batch_num, shuffle=False, num_workers=2, pin_memory=False,
                                  drop_last=False)
    ...
    model.load_state_dict(Temp_state_dict,False)
